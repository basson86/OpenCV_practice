...
// create histogram of gray-level images
...
public:
	...
	Histogram1D(){
		...
		hranges[0] = 0.0; // start from 0(inclusive) 
		hranges[1] = 256.0;  // end with 256 (exclusive)
		ranges[0] = hranges;
	...
	//Gets the number of bin in histogram
	int getNBins() 
	{
		return histSize[0];
	}
	...
	cv::Mat getHistogram(const cv::Mat & image)
	{
		...
		// Computer 1D histogram with calcHist
		cv::calcHist(&image,
			1, // histogram of 1 image only
			channels, // the channel used
			cv::Mat(), // no mask is used
			hist, // the result histogrm
...
// static methods
	...
	// Applies a loopup table transforming an input image into a 1-channel image
